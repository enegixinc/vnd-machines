<template>
    <div>
        <TheBreadcrumbs :current-location="$t('links.newCategories')" />
        <div class="panel pb-0 mt-6">
            <div class="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                <h5 class="font-semibold text-lg dark:text-white-light">{{ $t('categoriesPages.addNewCategory') }}</h5>
            </div>
            <form @submit="onSubmit" class="pt-5 pb-10">
                ...
                <submit-button :label="$t('categoriesPages.addNewCategoryButton')" :loading="loading">
...
<script setup lang="ts">
    ...
    const { handleSubmit, resetForm, setValues } = useForm<CreateCategoryDto>({
