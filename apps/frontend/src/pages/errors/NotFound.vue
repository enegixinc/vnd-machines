    ...
    <div class="relative flex min-h-screen items-center justify-center overflow-hidden">
        ...
        >
            <div class="relative">
                ...
                />
                <p class="mt-5 text-base dark:text-white">{{ $t('errors.thePageYouRequestedWasNotFound') }}</p>
                <router-link :to="{ name: 'home' }" class="btn btn-gradient mx-auto !mt-7 w-max border-0 uppercase shadow-none">{{
                    $t('links.home')
                }}</router-link>
            </div>
        </div>
