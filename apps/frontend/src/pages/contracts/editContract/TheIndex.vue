    ...
    <div>
        ...
        <div class="panel pb-0 mt-6">
            <div class="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                <h5 class="font-semibold text-lg dark:text-white-light">{{ $t('contractsPages.editContract') }}</h5>
            </div>
            <form @submit="onSubmit" class="pt-5 pb-10">
                ...
                <submit-button :label="$t('contractsPages.updateContract')" :loading="loading" />
            </form>
            <div class="pb-5"></div>
        </div>
    </div>
...
const { loading, getOneEntity, t,handleEmptyLang, cleanResource, updateEntity } = useContract({
});
...
const { handleSubmit, resetForm, setValues, values:allValues } = useForm<CreateContractDto>({
...
const onSubmit = handleSubmit(
    (values) => {
        ...
        updateEntity({
            id:id,
            requestBody:{
                feePerSale: cleanedData.feePerSale,
                supplier: cleanedData.supplier,
                feeType: cleanedData.feeType,
                status: cleanedData.status,
                description: cleanedData.description,
                startDate:cleanedData.startDate ,
                endDate: cleanedData.endDate,
            }
        },t('contractsPages.TheContractHasBeenSuccessfullyUpdated'))
    },
    () => {}
);
getOneEntity(
