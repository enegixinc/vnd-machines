<template>
    <div :class="{ 'has-error': errorMessage, 'has-success': meta.dirty && meta.valid }">
        ...
        <input
            class="form-input ltr:text-left rtl:text-right"
            :type="type || 'text'"
            v-model.trim="value"
            @change="handleChange"
...
<script setup lang="ts">
    ...
    const { handleChange, value, handleBlur, errorMessage, meta, setValue } = useField(() => data.name, undefined);

    //@ts-ignore
    const textareaValue: string = data.textArea ? value : undefined;
    watch(value, (newVal) => {
        if (data.type === 'number') {
            const newNumber = parseFloat(newVal as string);
            if (!Number.isNaN(newNumber) && newNumber > 0) {
                setValue(newNumber, false);
            } else {
                setValue(0, false);
            }
        }
